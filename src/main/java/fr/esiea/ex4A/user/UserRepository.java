...
public class UserRepository {
    ...
    final boolean userAlreadyExist(User user){
        return this.userList.containsKey(user.getUserId());
    }

    final User getUser(String userName, String countryId){
        return this.userList.get(userName.concat(countryId));
    }

    final ArrayList<Matches> matchUser(User userData){
        final ArrayList<Matches> matchingUsers = new ArrayList<>();
